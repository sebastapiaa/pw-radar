...
|---|---|---|---|
| industry | `industryFilter` = `search_companies` subset with `industryList` / `companyTypeList` | `education` (industries education, education.k12, education.university, or company type education; Seb's decision 2026-09-17) → tag `excluded:education`; `government` (industry government or company type government; Seb's decision 2026-09-18) → tag `excluded:government`; `security_vendor` (software.security, bizservice.security) → tag `excluded:security-vendor`; `msp_candidate` (bizservice.techconsulting **and** a provider-style name) → tag `partner:candidate` | `it_services_industry` (bizservice.techconsulting without a provider name), `name_suggests_provider` |
...
| distress | stored `search_scoops` M&A / Divestiture / Layoffs (daily "distress" group; `DISTRESS_TYPES` in `scoring.ts` give them zero strength, no stack, no why-now) | `acquired`, `bankruptcy`, `shutdown` when the headline says the company is the target | `distress_signal` (layoffs, divestiture, M&A as acquirer or ambiguous) |
